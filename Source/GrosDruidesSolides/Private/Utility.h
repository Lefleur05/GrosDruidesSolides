...
#define TIMER_PARAM(worldContext, handleVariable, lambdaBody, delay) if (!worldContext || !worldContext->GetWorld()) return; worldContext->GetWorld()->GetTimerManager().SetTimer(handleVariable, FTimerDelegate::CreateLambda([=, this]() { lambdaBody; }), delay, false);
...
#define TIMER_LOOP_PARAM(worldContext, handleVariable, lambdaBody, initialDelay, loopDelay) if (!worldContext || !worldContext->GetWorld()) return; worldContext->GetWorld()->GetTimerManager().SetTimer(handleVariable, FTimerDelegate::CreateLambda([=, this]() { lambdaBody; }), loopDelay, true, initialDelay);
...
#define FRAME_LOG_CUSTOM_C(message, color) UKSL::PrintString(this, FPaths::GetBaseFilename(TEXT(__FILE__)) + "(" + FString::FromInt(__LINE__) + ")=> " + message, true, false, color, DELTATIME)
// ======== LOGGING ========
...
#define LOG_SERVER_CLIENT(message) UKSL::PrintString(this, FPaths::GetBaseFilename(TEXT(__FILE__)) + "(" + FString::FromInt(__LINE__) + ")=> " + message, true, false, (HasAuthority()? FColor::Red : FColor::Blue), 20.0f)
#define FRAME_LOG_SERVER_CLIENT(message)  UKSL::PrintString(this, FPaths::GetBaseFilename(TEXT(__FILE__)) + "(" + FString::FromInt(__LINE__) + ")=> " + message, true, false, (HasAuthority()? FColor::Red : FColor::Blue), DELTATIME)
